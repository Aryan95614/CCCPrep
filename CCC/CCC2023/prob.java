...
public class prob {
    static Vector<Vector<Integer>> matchUps(int[] main, int size, int domain) {
    ...
    public static int asymmetric(Vector<Vector<Integer>> main, int startingLocation, int domain) {

        int value = 0;

        Vector<Integer> location = main.get(startingLocation);
        System.out.println(location);

        for (int i = 0; i < location.size()/2; i++) {
            value += Math.abs(location.get(i) - location.get(domain-i-1));
        }
